...
import { getAllVendorProducts, enrichVendorProductForCard } from "@/lib/mock";
    ...
    <PageTransition className="mx-auto max-w-7xl px-4 py-8 lg:px-6">
      ...
      <div className="mt-6 flex items-center gap-5">
        ...
        <div>
          ...
          <p className="mt-1 text-sm text-slate-400">{products.length} vendor listings</p>
        </div>
      </div>
      <div className="mt-8 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
        {products.map((p) => <ProductCard key={p.id} product={p} variantSummary={p.variantSummary} />)}
      </div>
    </PageTransition>
  );
}
